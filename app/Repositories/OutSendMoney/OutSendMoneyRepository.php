...
class OutSendMoneyRepository extends Repository implements IOutSendMoneyRepository
{
    ...
    {
        return $this->model->where('created_at','<=',Carbon::now()->subDay())->where('status','=',1)->sum('total_amount');
    }
    ...
    {
        return $this->model->where('created_at','<=',Carbon::now()->subDay())->where('status','=',1)->sum('service_fee');
    }
